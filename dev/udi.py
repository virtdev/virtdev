...
class UDI(object):
    ...
    def _proc_tasklet(self, target, args, timeout):
        t = Tasklet(target=target, args=args, parent=self)
        return t.wait(timeout)

    def _proc(self, target, args, timeout):
        if TASKLET:
            return self._proc_tasklet(target, args, timeout)
        else:
            return self._proc_safe(target, args, timeout)
