...
class UDO(object):
    ...
    def _do_handle(self, device, buf):
    ...
    def _handle_safe(self, device, buf):
        try:
            self._do_handle(device, buf)
        except:
            log_err(self, 'failed to handle, type=%s, name=%s' % (device.d_type, device.d_name))

    def _handle_unsafe(self, device, buf):
        self._do_handle(device, buf)

    def _handle(self, device, buf):
        if SAFE:
            self._handle_safe(device, buf)
        else:
            self._handle_unsafe(device, buf)
